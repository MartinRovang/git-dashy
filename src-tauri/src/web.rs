...
mod tests {
    ...
    #[test]
    fn csp_frames_nothing_from_outside() {
        // the logo's YouTube player is gone, and with it the only reason to frame another origin
        assert!(!super::CSP.contains("frame-src"));
        assert!(!super::CSP.contains("youtube"));
    }
