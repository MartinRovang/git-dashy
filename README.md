...
`spaghetti-audit` spell; its writing counterpart, the `fog-audit` spell, audits the prose a PR adds. A new starter spell reaches existing installs too, once; a starter you delete stays deleted (except,
once, a `spaghetti-audit` or `fog-audit` deleted on an install first made on 2.36.0–2.37.0).
