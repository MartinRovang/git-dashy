...
**Where it applies is a separate question.** The board spans whatever the token can see, so with
nothing armed auto covers every repo on it. `gitdashy auto owner/name` arms one, and from then on
...
batch — what a newly armed repo already has listed is treated as seen, and only what arrives next
starts.
