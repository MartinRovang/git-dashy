...
def test_review_reads_memory_and_only_drafts_what_it_proposes(monkeypatch, tmp_path):
	...
	assert _counts(memory.drafts("a/b")) == [(1, "ci is slow, do not flag timeouts")]  # one review only drafts
...
def test_a_second_independent_review_turns_a_draft_into_a_fact(monkeypatch, tmp_path):
	...
	review(dict(PR), "opus")
	assert _counts(memory.drafts("a/b")) == [(1, "ci is slow, do not flag timeouts")]
	review(dict(PR), "opus")
