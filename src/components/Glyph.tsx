...
const TINTED: Record<string, string> = { 'spaghetti-audit': 'spaghetti' }
export function Glyph({ kind, name, size = 14 }: { kind: keyof typeof ICON; name: string; size?: number }) {
  if (TINTED[name]) return <img className="nglyph sprite tint" src={`/sprites/${TINTED[name]}.png`} width={size} height={size} alt="" />
