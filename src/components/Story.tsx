...
export function Story({ login, every, onUnfollow }: { login: string; every: number; onUnfollow: () => void }) {
      ...
      .then(async (r) => {
        if (r.ok) {
          ...
          if (last.current && next.at !== last.current.at) {
            const fresh = moved(last.current, next)
  ...
  useEffect(() => {
    if (!open) return
    const away = (e: MouseEvent) => el.current?.contains(e.target as Node) || setOpen(false)
  ...
  return (
    <div ref={el} className={`chip${news.length ? ' news' : ''}${open ? ' on' : ''}`}>
      ...
      {open ? (
        <div className="pop" role="dialog" aria-label={`${login}'s story`}>
          <div className="poph">
            <b>{login}</b>
            <div style={{ flex: 1 }} />
