...
export type Table = { name: string; change: string; refs: string[]; columns: { name: string; change: string; note: string }[] }
...
export function clean(db: DbImpact | null | undefined): { tables: Table[]; risks: Risk[] } {
  ...
  const tables = once(
    objs(d.tables).map((t) => ({
      ...
      columns: once(objs(t.columns).map((c) => ({ name: str(c.name), change: str(c.change), note: str(c.note) }))),
    })),
  )
