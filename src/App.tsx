...
import { ALL, FOLDABLE, NOBODY, type Only, UNFOLDED, buckets, flat, forView, groups, inBucket, isRead, isRefetching, isReviewed, onScreen, pick, pickBucket, pickable, type Pressed, refreshDone, remember, toggleHidden, visible, walkBucket, whoIs } from './board'
...
export default function App() {
  ...
  // f was pressed and its fetch has not answered yet: the ⟳ spins
  ...
  useEffect(() => {
    ...
    if (data.fetching && !pressed.seen) setPressed({ ...pressed, seen: true })
    else if (refreshDone(pressed, data)) setPressed(null)
  }, [data, pressed])
  const onRefresh = () => {
    setPressed({ fetchedAt: data?.fetchedAt ?? null, error: data?.error ?? '', seen: false })
    void call('/api/refresh', {}, 'refreshing…').then((ok) => ok || setPressed(null))
  }
  const onAuto = async () => {
    const on = !data?.auto
